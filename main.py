...
def write_sections_to_docx(sections, output_dir):
...
def load_parse_and_convert_document(file_path, output_dir):
    sections = {}
    ...
    write_sections_to_docx(sections, output_dir)
...
for filename in os.listdir(input_dir):
    if filename.endswith('.docx'):
        ...
        print(trunc_name)
        load_parse_and_convert_document(file_path, output_dir + '/' + trunc_name)
...
for subdir, _, files in os.walk(output_dir):
    ...
    for file in files:
        ...
        if file_path.endswith('.pdf'):
            ...
            append_text_to_docx(docx_path, f"This document refers ONLY to tools covered by troubleshooting document: {subdir.split('/')[-1]}")
